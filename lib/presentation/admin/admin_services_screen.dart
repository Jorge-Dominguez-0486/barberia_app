...
class _AdminServicesScreenState extends State<AdminServicesScreen> {
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<List<ServiceModel>>(
              stream: _repo.streamAllServices(),
              builder: (_, snapshot) {
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text('Error al cargar servicios', style: TextStyle(color: AppColors.error, fontSize: 16)),
                        const SizedBox(height: 12),
                        ElevatedButton(
                          onPressed: () => setState(() {}),
                ...
                if (_query.isNotEmpty) {
                  services = services.where((s) => s.nombre.toLowerCase().contains(_query)).toList();
                }
                ...
                if (services.isEmpty) {
                  return const Center(
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.content_cut_outlined, size: 48, color: AppColors.grey),
                        SizedBox(height: 12),
                        Text('No hay servicios disponibles', style: TextStyle(color: AppColors.grey, fontSize: 16)),
                      ],
                    ),
                  );
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, i) => _buildCard(context, services[i]),
                );
              },
            ),
          ),
          ...
          children: [
              ...
              child: service.imagenUrl != null
                  ? ClipRRect(
                      ...
                      child: Image.network(
                        ...
                        width: 56,
                        height: 56,
                        errorBuilder: (_, __, ___) => const Icon(Icons.content_cut, color: AppColors.gold),
                      ),
                    )
            ...
            Expanded(
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color: service.activo ? AppColors.success : AppColors.grey,
                            fontSize: 12,
                  ...
                  Text(
                    '\$${service.precio.toStringAsFixed(2)}  |  ${service.duracionMinutos} min',
            ...
            Column(
              children: [
                IconButton(
                  icon: Icon(
                    service.activo ? Icons.visibility : Icons.visibility_off,
                    color: AppColors.grey,
                    size: 20,
                  ),
                  onPressed: () {
                    _repo.updateService(service.id, {'activo': !service.activo});
                    showAppSnackBar(context, service.activo ? 'Servicio desactivado' : 'Servicio activado', type: SnackType.info);
                  },
                ),
                Row(
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (_, setDialogState) => AlertDialog(
          ...
          title: const Text('Agregar Servicio', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.bold)),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final id = _firestore.collection('services').doc().id;
                ...
                if (ctx.mounted) {
                  Navigator.pop(ctx);
                  showAppSnackBar(context, 'Servicio creado', type: SnackType.success);
                }
              },
  ...
  void _showEditDialog(BuildContext context, ServiceModel service) {
    ...
    final descCtrl = TextEditingController(text: service.descripcion);
    final precioCtrl = TextEditingController(text: service.precio.toString());
    final durCtrl = TextEditingController(text: service.duracionMinutos.toString());
    final imageUrlCtrl = TextEditingController(text: service.imagenUrl ?? '');
    bool activo = service.activo;
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (_, setDialogState) => AlertDialog(
          backgroundColor: AppColors.card,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: Text('Editar: ${service.nombre}', style: const TextStyle(color: AppColors.white, fontWeight: FontWeight.bold)),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final precio = double.tryParse(precioCtrl.text.trim()) ?? 0;
                final dur = int.tryParse(durCtrl.text.trim()) ?? 30;
                await _repo.updateService(service.id, {
                  'nombre': nombreCtrl.text.trim(),
                  'descripcion': descCtrl.text.trim(),
                  'precio': precio,
                  'duracionMinutos': dur,
                ...
                if (ctx.mounted) {
                  Navigator.pop(ctx);
                  showAppSnackBar(context, 'Servicio actualizado', type: SnackType.success);
                }
              },
              child: const Text('Guardar'),
            ),
  ...
  void _confirmDelete(BuildContext context, ServiceModel service) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: AppColors.card,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Eliminar servicio', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.bold)),
        content: Text(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (ctx.mounted) {
                Navigator.pop(ctx);
                showAppSnackBar(context, 'Servicio eliminado', type: SnackType.success);
              }
            },
